...
function test_convergence(formul, thicknessmult = 1/100, distortion = 0.0)
    ...
    ns = [8, 16, 32, 64, 128]
    for n in ns
        push!(results, _execute_dsg_model(formul, n, R*thicknessmult, false, 3*distortion/n))
    end
    return ns, results
end
...
let
    ...
    for distortion in [0.0, 1.0]
        ns, results100 = cos_2t_press_cylinder_free_examples.test_convergence(FEMMShellT3FFModule, 1/100, distortion)
        ns, results1000 = cos_2t_press_cylinder_free_examples.test_convergence(FEMMShellT3FFModule, 1/1000, distortion)
        ns, results10000 = cos_2t_press_cylinder_free_examples.test_convergence(FEMMShellT3FFModule, 1/10000, distortion)
        ns, results100000 = cos_2t_press_cylinder_free_examples.test_convergence(FEMMShellT3FFModule, 1/100000, distortion)
