...
function _execute_dsg_model(n = 8, visualize = true)
    ...
    if visualize
        ...
        vtkwrite("vis_scordelis_lo-$(femm.transv_shear_formulation)-$(n)-m.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
        scalars = []
        for nc in 1:3
        ...
        vtkwrite("vis_scordelis_lo-$(femm.transv_shear_formulation)-$(n)-n.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
        scalars = []
        for nc in 1:2
        ...
        vtkwrite("vis_scordelis_lo-$(femm.transv_shear_formulation)-$(n)-q.vtu", fens, fes; scalars = scalars, vectors = [("u", dchi.values[:, 1:3])])
